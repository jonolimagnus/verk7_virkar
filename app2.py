...
conn = pymysql.connect(host='tsuts.tskoli.is', port=3306, user='2611982589', password='mypassword', database='2611982589_vef2_v7')
...
def nyr():
    ...
    if  request.method == 'POST':
        ...
        try:        
            cur = conn.cursor()
            cur.execute("INSERT INTO 2611982589_vef2_v7.users(user_name, user_email, user_password) VALUES(%s, %s, %s)",(user, email, password))
            conn.commit()
            cur.close()
...
def login():
    ...
    if request.method == 'POST':
        ...
        conn = pymysql.connect(host='tsuts.tskoli.is', port=3306, user='2611982589', password='mypassword', database='2611982589_vef2_v7')
        cur = conn.cursor()
        cur.execute("SELECT count(*) FROM 2611982589_vef2_v7.users where user_name=%s and user_password=%s",(user,psw))
...
def users():
    cur = conn.cursor()
    resultValue = cur.execute("SELECT user_name FROM 2611982589_vef2_v7.users")  # * = allir dálkar
    if resultValue > 0:
...
def admin():
    if not session.get('logged_in'):
        ...
        try:
            cur = conn.cursor()
            resultValue = cur.execute("SELECT * FROM 2611982589_vef2_v7.users")  # * = allir dálkar
            if  resultValue > 0:
                userDetails = cur.fetchall()
                flash('velkomin')
